...
\newcommand{\repoOwn}{causal-wave-landings-repro}
...
$\Ngen = 3$ (in the precise sense made explicit in
Section~\ref{sec:reduction}: the Diophantine equation arising
from $C_1, C_4$ has the unique positive integer solution $N=3$
for the value $1/4$ --- this is a uniqueness fact about that
equation, \emph{not} a derivation of $\Ngen$ from internal
physics; $\Ngen=3$ remains an observational input of the
framework). \emph{We present $\mathcal R$ as a hypothesis about
...
and bundle \pyown{data/projector_support_variation.json}{projector_support_variation.json}).
...
\begin{remark}[Relation to the spectral-action / noncommutative-geometry programme]
Reading physical coefficients off the joint spectral measure
of an operator family has a direct precedent in the
noncommutative-geometry programme. The Chamseddine--Connes
spectral action~\cite{ChamseddineConnes1997} obtains the
Standard-Model action --- including the Yukawa couplings and
the Cabibbo--Kobayashi--Maskawa mixing matrix --- as spectral
invariants of the Dirac operator of a spectral triple
$(\mathcal A,\mathcal H,D)$, the finite part
$(\mathcal A_{F},\mathcal H_{F},D_{F})$ of which carries the
flavour data~\cite{ConnesChamseddineMarcolli2007}, rather than
postulating them as free Lagrangian parameters.
Theorem~\ref{thm:projector_basis} is methodologically kin: the
five System-$\mathcal R$ coefficients are read off the joint
spectral measure of the four-tuple
$\{X_{\xi},X_{\Omega},X_{\rm common},X_{\rm sync}\}$ rather
than fitted. The genuine distinction is the substrate ---
the spectral-action triple is a \emph{continuum} object,
whereas the present readout is a \emph{finite}
bounded-operator construction on a relational $\Xi$-lattice
whose coefficients are fixed by the two integer inputs
$(d,N_{\rm gen})$. Both programmes share the stance that the
Standard-Model parameters are spectral data of an operator
structure, not free inputs; the present construction is the
discrete, integer-counted instance of that stance.
\end{remark}
...
uniqueness; the paragraph confirms it empirically by testing
...
Three caveats apply, each handled in the cluster discussion of
the companion loop-class paper~\cite{Paper3}:
(i) the three observables are physically distinct sectors but
share a common loop-factor ansatz, so they are not
verified-independent at the level a strict Fisher combination
would require --- which is precisely why the companion paper
reports $P_{\mathrm{combined}}$ as a \emph{controlled-likelihood}
test rather than a strict $p$-value, the same framing adopted
here; (ii) the uniform-on-$[0,2.5\%]$ null is a modelling
choice, scanned against alternative null distributions by the
companion paper's bundled sensitivity analysis; (iii) the
cluster selection itself was made \emph{after} the post-loop
residuals were computed and is therefore conditional on the
...
(\pyown{src/reduction_null_v2.py}{reduction_null_v2}, output
\pyown{outputs/reduction_null_v2.json}{reduction_null_v2.json}). The corresponding
...
\scriptsize
\input{tables/tab_systemR_audit}
\normalsize
\end{table}
...
(\pyown{outputs/reduction_null_v2.json}{reduction_null_v2.json}). The complementary
...
\paragraph{Chirality-flip dual: matter-branch sign flip and
cone-mode extension in the emergent-time bundle.}
The vacuum--matter chirality flip propagates structurally into
the emergent-time bundle of the UV-closure technical
foundations~\cite{Paper6}: the same flip that takes
$(\alpha_{\xi},\gamma)\!\to\!(\gamma,\alpha_{\xi})$ between
vacuum-branch and matter-branch System-$\mathcal R$ values also
induces a sign flip and a $(d{+}1)$-mode extension in two
downstream emergent-time observables. \emph{(i)~Tunnel-parameter
$(SG)$-axiom anchoring.} The canonical-regime Sommerfeld tunnel
parameter
$\eta_{\rm somm}^{\rm cano}\!=\!N_{\rm gen}(1\!+\!\gamma^{2}d/(d{+}1))$
of \cite{Paper6}~(ET7) reflects the vacuum-branch spatial
fraction $d/(d{+}1)$ leakage; the extended-regime value
$\eta_{\rm somm}^{\rm ext}\!=\!N_{\rm gen}(1\!-\!\gamma\,\lambda_{\infty})$
of \cite{Paper6}~(ET8) is dressed directly by the $(SG)$-axiom
spectral gap $\lambda_{\infty}\!=\!(d{-}1)/(2d)\!=\!3/8$,
tying the post-flip matter-branch tunneling to the same
structural anchor as (M3) of the composition theorem.
\emph{(ii)~Tolman chirality-pair Pure-Self-Energy correction.}
The canonical Tolman gravitational-redshift residual
$\mathrm{Tolman}^{\rm cano}\!=\!1\!-\!1/(N_{\rm chir}(d{+}N_{\rm gen}))\!=\!13/14$
extends in the post-flip matter regime to
$\mathrm{Tolman}^{\rm ext}\!=\!\mathrm{Tolman}^{\rm cano}\!+\!\gamma^{2}\,N_{\rm chir}\!=\!13/14\!+\!1/50$,
a $\gamma^{2}$-order Pure-Self-Energy correction averaged over
the chirality pair (the same Lemma-$3$ form that dresses
$m_{d},m_{s}$ via $(1\!-\!N_{\rm chir}\gamma^{2})$). Both shifts are direct structural
consequences of the chirality flip
Eq.~\eqref{eq:p2_theta_running} and not independent fits; the
spectral-dimension residual $f_{\rm LC}\!=\!d_{\rm eff}\!-\!3\!=\!\log_{2}((2d{+}1)/(2d))\!=\!\log_{2}(9/8)$
remains regime-invariant (identical in canonical and extended
bundles), confirming that the chirality-flip transformation
operates on the matter-state and time-rate sectors but not on
the bit-logarithmic information measure of the
$(2d{+}1)$-element carrier basis.
...
$[0.27, 0.84]$ with a non-monotonic pattern. Quantitative
test of seven alternative C$_{2}$-like identities on the
$10$-regime ladder gave the following mean residuals:
...
\textbf{G: $D_{\Omega}\!=\!\beta_{\pi}(1\!-\!\gamma)+\gamma\alpha_{\xi}$} & $\mathbf{39.1\%}$ \\
H: $D_{\Omega}\!=\!(\beta_{\pi}-\gamma)+(\gamma-\alpha_{\xi})^{2}$ & $55.2\%$ \\
\bottomrule
\end{tabular}
\end{center}

The mixed-projection form G has the lowest mean residual on the
$10$-regime ladder ($39.1\%$); the vacuum-canonical form A
sits at $58.5\%$ and is no longer the best candidate once the
post-flip $N\!\ge\!256$ matter regimes are included.
No algebraic transformation of the $(\alpha_{\xi},
...
would require $X\!=\!1$ at vacuum and $X\!=\!-16.85$ at $N\!=\!300$
...
\pyown{src/verify_D_Omega_alternative_derivations.py}{verify_D_Omega_alternative_derivations}.

\paragraph{$D_{\Omega}$ lattice-resonance pattern: from the
period-$d$ correlation readout to the structural identification.}
The non-monotonic behaviour of $D_{\Omega}(N)$ on the canonical
$10$-regime ladder $N\!\in\!\{50,64,72,84,100,128,200,256,300,512\}$
shows the strongest negative deviations from any smooth
chirality-mixing form at $N\!=\!2^{9}\!=\!512$ (residual
$+0.565$), $N\!=\!2^{7}\!=\!128$ ($+0.543$), and
$N\!=\!2^{8}\!=\!256$ ($+0.450$). The lattice data underlying
the readout are bundled at
\pyown{data/causal_wave_per_N_readout.json}{causal_wave_per_N_readout.json}; the ladder was
generated by $24$-seed runs at $N\!\in\!\{64,72,84,100\}$,
$12$-seed runs at $N\!\in\!\{128,256,300,512\}$, and an $8$-seed
run at $N\!=\!200$ (total $152$ seeds across $9$ non-anchor
regimes; $N\!=\!50$ is the calibration anchor of the C1/C2/C3
constraint solve, no separate snapshot run).

An initial correlation analysis on the original $8$-point
ladder $\{50,\ldots,200,300\}$ identified $\log_{2}(N)\bmod d$
with $d\!=\!4$ as the strongest naive periodicity predictor of
the deviation $D_{\Omega}(N)\!-\!67/80$, with Pearson
$r_{8}^{(p=4)}\!=\!+0.818$, exceeding the Bott
$r_{8}^{(p=8)}\!=\!+0.621$ alternative. The extension to the
$10$-point ladder including $N\!=\!256$ ($\log_{2}\!=\!8$,
$\bmod d\!=\!0$, $v_{2}\!=\!8$) and $N\!=\!512$
($\log_{2}\!=\!9$, $\bmod d\!=\!1$, $v_{2}\!=\!9$) inverts the
correlation hierarchy:
\begin{center}
\begin{tabular}{c c c}
\toprule
period $p$ & Pearson $r_{8}$ (original) & Pearson $r_{10}$ (extended) \\
\midrule
$4\;(\!=d)$ & $+0.818$ & $+0.248$ \\
$7$         & $-0.450$ & $-0.540$ \\
$8\;(\mathrm{Bott}\,2d)$ & $+0.621$ & $+0.001$ \\
$10$--$16$  & $+0.263$ & $+0.527$ \\
\bottomrule
\end{tabular}
\end{center}
The two new high-$v_{2}$ points break the period-$d$ ansatz
directly: $N\!=\!256$ sits at the period-$d$ boundary
($\bmod d\!=\!0$) yet shows the third-deepest dip
($+0.450$) instead of the expected near-zero rebound; $N\!=\!512$
sits at $\bmod d\!=\!1$ where the period-$d$ hypothesis predicts
modest deviation, but observes the deepest dip of the entire
ladder ($+0.565$).
\begin{center}
\begin{tabular}{c c c c c}
\toprule
$N$ & seeds & $\log_{2}(N)$ & $\log_{2}(N)\bmod d$ & $67/80\!-\!D_{\Omega}$ \\
\midrule
$50$  & anchor & $5.644$ & $1.644$ & $-0.003$ \\
$64$  & $24$   & $6.000$ & $2.000$ & $+0.078$ \\
$72$  & $24$   & $6.170$ & $2.170$ & $+0.167$ \\
$84$  & $24$   & $6.392$ & $2.392$ & $+0.406$ \\
$100$ & $24$   & $6.644$ & $2.644$ & $+0.223$ \\
$128$ & $12$   & $7.000$ & $3.000$ & $+0.543$ \\
$200$ & $8$    & $7.644$ & $3.644$ & $+0.500$ \\
$256$ & $12$   & $8.000$ & $\mathbf{0.000}$ & $\mathbf{+0.450}$ \\
$300$ & $12$   & $8.229$ & $0.229$ & $+0.013$ \\
$512$ & $12$   & $9.000$ & $\mathbf{1.000}$ & $\mathbf{+0.565}$ \\
\bottomrule
\end{tabular}
\end{center}
The deepest dips on the extended ladder coincide with the highest
$2$-adic valuations ($v_{2}(128)\!=\!7$, $v_{2}(256)\!=\!8$,
$v_{2}(512)\!=\!9$); the rebound at $N\!=\!300$
($v_{2}\!=\!2$) is the lowest-$v_{2}$ post-flip point on the
ladder, not a period-$d$ wrap. The structural reading is therefore
not a periodicity in $\log_{2}(N)$ but a $2$-adic valuation
interaction with the post-flip chirality phase, captured by
Eq.~\eqref{eq:p2_D_Omega_2adic_chirality} below. The period-$d$
narrative is retained as the historical empirical motivator
that pointed the search toward $\log_{2}(N)$-arithmetic
predictors; the post-extension correlation hierarchy and the
geometric reading that connects $2$-adic valuation to lattice
binary-subdivision modes are the load-bearing structural
identification.

\paragraph{Connection to chirality flip and matter sector.}
The lattice-resonance structure of $D_{\Omega}$ has a natural
geometric reading that connects to the chirality-flip
framework~\eqref{eq:p2_theta_running}. Each factor of $2$ in $N$
adds one binary-subdivision level to the lattice modes; the
$2$-adic valuation $v_{2}(N)$ counts how many such levels are
present before the remaining odd factor enters. The
Cl$(1,3)$ diffusion-trace operator is sensitive to this binary
arithmetic --- cyclotomic eigenvalues
$4\sin^{2}(\pi j/N)$ at $N\!=\!2^{k}$ are algebraic numbers in
$\mathbb{Q}(\zeta_{2^{k}})$ with exact heat-kernel-cancellation
identities --- so the deviation grows with $v_{2}(N)$ rather
than running on a fixed period in $\log_{2}(N)$.

The chirality angle $\theta_{\rm chir}(N)$ runs continuously with
$\ln(N)/\ln(d N_{\rm gen})$, while the $D_{\Omega}$
lattice resonance is modulated by the discrete arithmetic of
$v_{2}(N)$ post-flip. The two scales differ:
\begin{itemize}
\item Chirality flip: full vacuum-to-inversion sweep over a
  factor $d N_{\rm gen}\!=\!12$ in $N$ (one e-fold of
  $\ln(12)\!\approx\!2.485$).
\item Lattice resonance: amplitude $\propto v_{2}(N)$ post-flip,
  growing unbounded along the binary subsequence
  $N\!=\!2^{k}$ and vanishing at odd $N$ (no enhancement).
\end{itemize}
...
lattice resonance amplitude is the product
$\sin^{2}\theta_{\rm chir}\,v_{2}(N)$, so the resonance turns on
only after the chirality has rotated into the matter regime and
is then weighted by the $2$-adic factorisation of $N$. This
explains the deepest dips on the ladder
($N\!=\!128, 256, 512$ with $v_{2}\!=\!7, 8, 9$ post-flip) and
the rebound at $N\!=\!300$ ($v_{2}\!=\!2$, the lowest-$v_{2}$
post-flip point on the ladder). Pure powers of $2$ at deep-matter
chirality therefore extend the suppression unbounded in
$v_{2}$ within the linear-extrapolation regime; no period-$d$
``wrap to vacuum'' returns the trace to $67/80$ once the
chirality is saturated.
...
against two framework matter-core indicators (top-decile of
the per-node energy density $t_{00}(a)$ and top-decile of the
closure residual $\Delta(a)$) on $23{,}632$ lattice nodes from
the canonical $d1$ $\mathcal{P}_{5}$ N-ordered ladder at
$N\!=\!64,72,84,100,128,200,256,300,512$ (nine regimes;
alt-anchor $\mathcal{P}_{6}/\mathcal{P}_{8}$ regimes excluded
per the alt-anchor separation rule, since auditing the
chirality-flip running formula requires a homogeneous N-ordered
pool). Pooled AUC$\,=\!0.841\,[0.833,0.849]$ and matter-core
enrichment ratio
$P(C_{N}|\Delta\theta\!>\!0)/P(C_{N}|\Delta\theta\!\leq\!0)\!=\!10.19\!\times$
against $t_{00}(a)$. Per-regime AUC ranges $0.80$--$0.89$ across
all nine regimes (peak $0.886$ at $N\!=\!72$, slight downturn
to $0.798$ at the deep post-flip endpoint $N\!=\!512$) with
regime-relative ratios $6.5$--$23\!\times$. Leave-one-regime-out
($\Xi$-row-variance is the top candidate in $9/9$ folds)
confirms the result is non-random and not within-sample. The
chirality-flip therefore acts both as the global slow envelope
of the bounded-operator readouts and as a strong local
matter-core indicator at the per-node level. Detailed audit in
the companion paper on anisotropic source dynamics.

\textbf{Lattice-resonance refinement of the period-$d$ ansatz.}
The earlier period-$d$ hypothesis (uncorrected $p\!\sim\!0.045$,
Bonferroni-corrected for $7$ candidate periods $p\!\sim\!0.32$,
LOO RMS $\sim\!100\%$ of the data spread on the original
$8$-point ladder) is superseded by a simpler structural form
on the 10-regime canonical-physics ladder:

\begin{equation}
r(N) \;:=\; D_{\Omega}(N) \,-\, D_{\rm slow}(\theta_{\rm chir}(N))
\;=\; b_{0}\,+\,b_{1}\,\sin^{2}\!\theta_{\rm chir}(N)\,\cdot\,v_{2}(N),
\label{eq:p2_D_Omega_2adic_chirality}
\end{equation}

where $v_{2}(N)$ is the $2$-adic valuation of $N$ (number of
factors of $2$ in $N$). The interaction term
$\sin^{2}\!\theta\,v_{2}$ is the unique single structural
variable that combines the post-flip chirality phase with the
lattice's binary-mode arithmetic: pure powers of $2$ at
pre-flip $N\!=\!64$ ($v_{2}\!=\!6$, $\sin^{2}\!\theta\!=\!0.15$)
show no dip, post-flip $N\!=\!300$ with low $v_{2}\!=\!2$ also
shows no dip ($r\!=\!+0.02$), while $N\!=\!128,200,256,512$
(post-flip plus $v_{2}\!\geq\!3$) show consistently deep dips.
On the $10$-regime ladder the interaction-only fit gives $b_{0}
\!=\!-0.161, b_{1}\!=\!-0.052\!\approx\!-\gamma/2$, with
$R^{2}\!=\!0.369$, AIC $=\!-32.21$ and LOO-RMSE $0.192$. The
model comparison with the period-$d$ ansatz (AIC $=\!-28.72$,
LOO-RMSE $0.250$) and the full $4$-parameter combined model
(AIC $=\!-28.59$, LOO-RMSE $0.334$) selects this single
interaction term as the AIC- and LOO-winner; the Bonferroni
multiple-testing concern over $7$ candidate periods is replaced
by a single $2$-variable structural identification, removing
the prior statistical caveat.

The mechanistic interpretation is cyclotomic: at $N\!=\!2^{k}$
the discrete Laplacian eigenvalues $4\sin^{2}(\pi j/N)$
(see~\cite{Karlsson2012} for the explicit one-dimensional
heat-kernel on $\mathbb{Z}_{N}$) are algebraic numbers in
$\mathbb{Q}(\zeta_{2^{k}})$, generating exact heat-kernel-
cancellation identities that suppress the diffusion-trace
operator at post-flip phases where the matter-side sine
dominates. The heat-trace asymptotics on a torus are known to
be sensitive to the number-theoretical structure of the
lattice parameter under a Diophantine condition: rational versus
irrational arguments give qualitatively different small-$t$
contributions to the trace sum~\cite{GIV2006}. The
$d$-dimensional heat-kernel-trace factorisation we use in the
reproducers below decomposes as the Kronecker sum of the
one-dimensional traces by the recent theorem
of~\cite{LinWan2024}. As the lattice sizes $N_{i}\to\infty$,
the combinatorial-Laplacian determinant on the discrete torus
converges to its zeta-regularised continuum value with
arithmetic-dependent subleading
corrections~\cite{ChintaJK2010}; these subleading corrections
are the math-precise version of the
$\sin^{2}\theta_{\rm chir}(N)\,v_{2}(N)$ residual on our finite
canonical-physics ladder. Reproducers
\pyown{src/verify_D_Omega_2adic_chirality_phase.py}{verify_D_Omega_2adic_chirality_phase} (interaction
fit, AIC + LOO model selection) and
\pyown{src/verify_D_Omega_discrete_laplacian_spectrum.py}{verify_D_Omega_discrete_laplacian_spectrum}
(no smooth $1$D spectral metric improves on the discrete
$v_{2}$ predictor, confirming the cyclotomic-Galois reading
rather than a smooth-eigenvalue diagnostic).

Beyond the leading $\sin^{2}\!\theta\,v_{2}$ lattice-resonance,
the residual is expected to admit the same chirality-harmonic
basis $\sin(2\theta),\sin(4\theta),\sin(6\theta)$ that closes
the matter-localised $K, Q$ factor fields (companion paper P4-B,
five-coefficient harmonic closure at $<\!1\sigma$ per
coefficient). Under the framework's $\gamma^{2}$-universality
of chirality-flip-shift corrections (P4-B Section
\textit{Q\textsubscript{post} $\gamma^{2}$-scale derivation}),
these harmonic coefficients are expected to be $\gamma^{2}$-scale
System-$\mathcal{R}$ rationals with amplitude
$\mathcal{O}(\gamma^{2})\!=\!0.01$, two orders of magnitude
smaller than the $\mathcal{O}(\gamma\,v_{2})\!\sim\!0.5$
lattice-resonance amplitude that dominates at $N\!\leq\!512$. A
clean rational identification of the harmonic sub-leading
content will become possible once the queued $\mathcal{P}_{5}N$
lattices at $N\!=\!700,800,1024$ finish. The first such test
point, the $N\!=\!600$ single-seed trial, measured
$D_{\Omega}(600)\!=\!0.265$ against the linear-model prediction
$0.488$ ($45.7\%$ low), which is naturally resolved by revising
the slow-envelope matter-anchor from continuum $\pi/d$ to the
finite-$N$ lattice value $\pi/d\!-\!(d{+}1)\gamma\!\approx\!0.285$
(Eq.~\eqref{eq:D_post_lattice_finite_N} below). With this
revised anchor and the refit lattice-resonance coefficients
$b_{0}\!=\!-0.050$, $b_{1}\!=\!-0.0083$ on the
$11$-point ladder, the updated predictions become
$D_{\Omega}(700)\!\approx\!0.26$,
$D_{\Omega}(800)\!\approx\!0.23$,
$D_{\Omega}(1024)\!\approx\!0.18$ (with deeper $v_{2}\!=\!10$
modulation than $N\!=\!700,800$), converging toward
$D_{\Omega}^{\rm post,lattice}\!=\!0.285$ at the highest
saturated-chirality $N$ values. The deeper-than-anchor
modulation at $v_{2}\!\geq\!10$ is the residual lattice
resonance signature that the campaign will discriminate.
...
$D_{\Omega}$-running structure identified by
Eq.~\eqref{eq:p2_D_Omega_2adic_chirality} maps cleanly onto
this decomposition:
\begin{equation}
D_{\Omega}(N)
...
Eq.~\eqref{eq:p2_theta_running}, and the fast lattice-resonance
\begin{equation}
r(N)
   \;=\; b_{0} \;+\; b_{1}\,\sin^{2}\!\theta_{\rm chir}(N)\,v_{2}(N),
\qquad
b_{0}\!=\!-0.161,\;\; b_{1}\!\approx\!-\gamma/2,
\label{eq:p2_D_Omega_fast}
\end{equation}
with $v_{2}(N)$ the $2$-adic valuation of $N$. The slow envelope
runs over a factor $d N_{\rm gen}\!=\!12$ in $N$ (one chirality
flip); the fast lattice-resonance is structurally tied to the
arithmetic of $N$ (large at $N\!=\!2^{k}$, zero at odd $N$) and
scales with the post-flip chirality fraction $\sin^{2}\theta_{\rm chir}$,
so the dip pattern is read off the prime factorisation of $N$
rather than from a phenomenological bump function. The
single-cycle readout
$A\,f(\log_{2}N\!\bmod d)$ examined in earlier diagnostic work
is the finite-$N$ projection of Eq.~\eqref{eq:p2_D_Omega_fast}
onto a single cycle of $v_{2}$ values and is structurally
superseded by the present identification.

\textbf{Quantitative high-$N$ predictions} from
Eq.~\eqref{eq:p2_D_Omega_fast_slow}, using the
$10$-regime-fit coefficients $b_{0}\!=\!-0.1613$,
$b_{1}\!=\!-0.0522$ (extrapolated linearly into the high-$v_{2}$
regime; reproducer output
\pyown{outputs/verify_D_Omega_2adic_chirality_phase.json}{verify_D_Omega_2adic_chirality_phase.json}):
\begin{center}
\begin{tabular}{r r r r r r}
\toprule
$N$ & $v_{2}(N)$ & $\theta_{\rm chir}$ & $D_{\Omega}^{\rm slow}$ & $r(N)$ & $D_{\Omega}^{\rm pred}$ \\
\midrule
$256$  & $8$  & $54.7^{\circ}$ & $0.803$ & $-0.440$ & $\mathbf{0.363}$ \\
$512$  & $9$  & $69.0^{\circ}$ & $0.792$ & $-0.571$ & $\mathbf{0.221}$ \\
$\mathbf{600}$  & $\mathbf{3}$  & $\mathbf{71.6^{\circ}}$ & $0.791$ & $-0.302$ & $\mathbf{0.488}$ pred \\
\multicolumn{6}{l}{\hspace{0.5em}\textit{empirical (single-seed trial): }$D_{\Omega}(600)\!=\!0.265$ at $-45.7\%$ vs linear pred} \\
$1024$ & $10$ & $78.3^{\circ}$ & $0.788$ & $-0.662$ & $\mathbf{0.126}$ \\
$1536$ & $9$  & $81.7^{\circ}$ & $0.787$ & $-0.621$ & $\mathbf{0.165}$ \\
$2048$ & $11$ & $83.6^{\circ}$ & $0.786$ & $-0.728$ & $\mathbf{0.058}$ \\
$3072$ & $10$ & $85.5^{\circ}$ & $0.786$ & $-0.680$ & $\mathbf{0.106}$ \\
$4096$ & $12$ & $86.5^{\circ}$ & $0.786$ & $-0.785$ & $\mathbf{0.000}$ \\
\bottomrule
\end{tabular}
\end{center}
Critical features:
\begin{itemize}
\item \textbf{No vacuum rebound at large $N\!=\!2^{k}$.} The
  ``rebound to $67/80$'' naive period-$d$ reading is rejected by
  the structural form Eq.~\eqref{eq:p2_D_Omega_fast}: every
  power of $2$ past the chirality flip carries a $v_{2}$
  enhancement of the resonance, driving $D_{\Omega}$ further
  \emph{below} the slow envelope, not back toward $67/80$.
\item \textbf{$N\!=\!600$ single-seed trial reveals a finite-$N$
  matter-anchor correction.}
  The $N\!=\!600$ measurement (single-seed,
  \path|results_d1_p5n600_trial1seed|) gives
  $D_{\Omega}(600)\!=\!0.265$, $45.7\%$ below the linear-model
  prediction $0.488$. The pattern across the post-flip ladder
  $\{N\!=\!128,200,256,512,600\}$ shows the empirical
  $D_{\Omega}$-values clustering around $0.27\!\pm\!0.06$ ---
  far below the continuum matter anchor $\pi/d\!=\!0.785$ used
  in the slow-envelope Eq.~\eqref{eq:p2_D_Omega_slow}. A direct
  joint fit on the eleven-point ladder
  $\{N\!=\!50,\ldots,600\}$ on post-flip points ($\sin^{2}\theta\!>\!0.5$)
  identifies three roughly equivalent framework-rational
  candidates for the lattice-specific matter anchor:
  $\gamma N_{\rm gen}\!=\!3/10\!=\!0.300$,
  $\pi/(d N_{\rm gen})\!=\!\pi/12\!\simeq\!0.262$, and
  $2/(d{+}N_{\rm gen})\!=\!2/7\!\simeq\!0.286$. Each reduces the
  post-flip RMS residual by $\sim\!50\%$ relative to the $\pi/d$
  reading. The structurally cleanest form is
  \begin{equation}
  D_{\Omega}^{\rm post,lattice}
     \;=\; \tfrac{\pi}{d}\!-\!(d{+}1)\,\gamma
     \;=\; \tfrac{\pi}{d} - \tfrac{1}{2}
     \;\simeq\; 0.285,
  \label{eq:D_post_lattice_finite_N}
  \end{equation}
  the continuum matter value reduced by the Lorentzian-cone-
  summed carrier-defect leakage $(d{+}1)\,\gamma\!=\!5\gamma\!=\!1/2$.
  The $(d{+}1)\,\gamma$ correction reads physically as the
  cumulative $\gamma$-order leakage across the $(d{+}1)$
  Lorentzian-cone directions of the discrete light cone of
  Section~\ref{sec:p2_fast_slow_D_Omega}~(ET2 in the technical
  foundations~\cite{Paper6}); $\pi/d$ remains the strict
  continuum $N\!\to\!\infty$ asymptote.

  Under the revised slow envelope
  $D_{\Omega}^{\rm slow,revised}(\theta)\!=\!(67/80)\cos^{2}\theta\!+\!(\pi/d\!-\!(d{+}1)\gamma)\sin^{2}\theta$,
  the lattice-resonance residual $r(N)\!=\!D_{\Omega}^{\rm obs}\!-\!D_{\Omega}^{\rm slow,revised}$
  is small at fully-saturated post-flip ($\sin^{2}\theta\!\to\!1$:
  $r\!\to\!-0.08$) and largest at mid-flip
  ($\sin^{2}\theta\!\approx\!0.4$: $r\!\approx\!-0.34$ at $N\!=\!128$).
  The $N\!=\!600$ data point ($\sin^{2}\theta\!=\!0.90$, $v_{2}\!=\!3$)
  fits with residual $r\!\approx\!-0.08$, naturally explained
  without invoking a non-linear chirality-amplification mechanism.
  The $v_{2}\sin^{2}\theta$ linear ansatz of
  Eq.~\eqref{eq:p2_D_Omega_2adic_chirality} remains the dominant
  finite-$N$ residual structure; what the $N\!=\!600$ trial
  identifies is that the slow-envelope matter anchor itself
  carries a $(d{+}1)\gamma\!=\!1/2$ finite-$N$ correction below
  the continuum $\pi/d$ value, falsifying the original
  ``rebound to $\pi/d$'' reading and confirming the
  ``post-flip plateau at $\!\approx\!0.27$'' empirical pattern.

  Single-seed trial has typical per-seed variance $\sim\!15\%$;
  the $12$-seed campaign (\path|results_d1_p5n600_12seeds/|, in
  progress) and the queued $N\!\in\!\{700,800,1024\}$ ladder
  will discriminate between the three candidate lattice-anchor
  rationals and tighten the $(d{+}1)\gamma$ correction's
  numerical certificate.
\item \textbf{$N\!=\!4096$ as the strongest test point.} The
  post-flip chirality is saturated ($\sin^{2}\theta\!\to\!1$)
  and $v_{2}\!=\!12$ gives a nominal $r\!\approx\!-0.79$. Given
  the $N\!=\!600$ trial showing factor-$\sim\!2$ excess
  suppression at saturated $\sin^{2}\theta$, the actual
  $D_{\Omega}(4096)$ may be even lower than the linear
  extrapolation $\approx\!0$; a measured
  $D_{\Omega}(4096)\!\gtrsim\!0.3$ would falsify the
  lattice-resonance mechanism on canonical-physics lattices.
\item \textbf{Odd-$N$ vs power-of-$2$ contrast.} At any
  post-flip odd $N$ ($v_{2}\!=\!0$) the residual reduces to
  $r\!=\!b_{0}\!\approx\!-0.16$ irrespective of size, so
  $D_{\Omega}\!\approx\!0.63$; this gives a clean direct test
  against the deep-dip pattern at $N\!=\!2^{k}$.
\end{itemize}
\textbf{Caveat on extrapolation.} The fit was trained on
$10$ canonical-physics points at $N\!\in\![50,512]$ with
$v_{2}\!\in\![0,8]$; the $N\!\in\![1024,4096]$ predictions
extrapolate $v_{2}$ to $12$, a $50\%$ extension beyond the
training range. Saturation effects (an effective $D_{\Omega}$
floor or a non-linear $v_{2}$ kernel) may modify the linear
projection at the deepest power-of-$2$ points; this is part of
the falsification chain. The fast-slow decomposition of
$D_{\Omega}(N)$ is summarised graphically in
Fig.~\ref{fig:p2_fast_slow_decomposition}. Reproducers:
\pyown{src/verify_D_Omega_2adic_chirality_phase.py}{verify_D_Omega_2adic_chirality_phase} (structural
identification, AIC + LOO model selection, predictions at
$N\!\in\!\{256,\ldots,4096\}$),
\pyown{src/verify_D_Omega_lattice_resonance.py}{verify_D_Omega_lattice_resonance},
\pyown{src/verify_D_Omega_discrete_laplacian_spectrum.py}{verify_D_Omega_discrete_laplacian_spectrum}.
...
(B)~Slow chirality-mixing envelope. The continuum-limit form
$D_{\Omega}^{\rm slow,cont}\!=\!(67/80)\cos^{2}\theta\!+\!(\pi/d)\sin^{2}\theta$
transitions between vacuum $67/80\!=\!0.838$ and the Symanzik
$N\!\to\!\infty$ matter asymptote $\pi/d\!=\!0.785$; the
finite-$N$ lattice anchor identified by the $N\!=\!600$ trial
analysis (Eq.~\eqref{eq:D_post_lattice_finite_N}) is
$D_{\Omega}^{\rm post,lattice}\!=\!\pi/d\!-\!(d{+}1)\gamma\!\simeq\!0.285$,
$(d{+}1)\gamma\!=\!1/2$ smaller than the continuum value due to
the Lorentzian-cone-summed carrier-defect leakage. (C)~Fast lattice-resonance
$r(N)\!=\!b_{0}\!+\!b_{1}\sin^{2}\theta_{\rm chir}\,v_{2}(N)$
with $b_{0}\!\approx\!-0.16$, $b_{1}\!\approx\!-\gamma/2$;
deepest suppression at large $N\!=\!2^{k}$ where
$v_{2}\!\geq\!8$ (powers-of-$2$ marked dotted purple).
(D)~Total prediction
$D_{\Omega}^{\rm pred}\!=\!D_{\Omega}^{\rm slow}\!+\!r(N)$
overlaid with $10$-regime lattice data (orange circles) and
structural predictions at $N\!\in\!\{256,\ldots,4096\}$
(blue stars). The lattice-observed deep dip at $N\!=\!128$ is
reproduced by $v_{2}\!=\!7$. The predicted suppression at
$N\!=\!4096$ ($D_{\Omega}\!\approx\!0$ in the linear
extrapolation) and the contrast against odd-$N$ shoulders are
...
\path|verify_chirality_running_first_principles.py|.
The 8-regime per-$N$ data set is bundled at
\pyown{data/causal_wave_per_N_readout.json}{causal_wave_per_N_readout.json}.
...
Communications in Mathematical Physics \textbf{186}, 731 (1997);
arXiv:hep-th/9606001.
The Standard-Model action obtained as a spectral invariant of
the Dirac operator of a spectral triple; the methodological
precedent for reading physical coefficients off an operator
spectrum rather than postulating them.

\bibitem{ConnesChamseddineMarcolli2007}
A.~H.~Chamseddine, A.~Connes, and M.~Marcolli,
\emph{Gravity and the standard model with neutrino mixing},
Advances in Theoretical and Mathematical Physics \textbf{11},
991 (2007); arXiv:hep-th/0610241.
The finite spectral triple $(\mathcal A_{F},\mathcal H_{F},
D_{F})$ whose Dirac operator $D_{F}$ encodes the Yukawa
couplings and CKM mixing of the Standard Model.
...
\bibitem{GIV2006}
V.~Gayral, B.~Iochum and D.~V.~Vassilevich,
\emph{Heat kernel and number theory on NC-torus},
Commun.\ Math.\ Phys.\ \textbf{273}, 415--443 (2007),
arXiv:hep-th/0607078.
Establishes that heat-trace asymptotics on the (non-commutative)
torus are sensitive to the number-theoretical structure of the
deformation parameters under a Diophantine condition; rational
versus irrational deformation parameters give qualitatively
different small-$t$ behaviour. Direct mathematical anchor for
the lattice-arithmetic ($v_{2}(N)$) sensitivity of the
discrete-Laplacian heat-trace on $\mathbb{Z}_{N}^{d}$ exploited
in the 2-adic chirality-phase residual identification of
Eq.~\eqref{eq:p2_D_Omega_2adic_chirality}.

\bibitem{Karlsson2012}
A.~Karlsson,
\emph{Heat-kernels on the discrete circle and interval},
arXiv:1207.2096 (2012).
Explicit heat-kernel formula on the discrete circle $\mathbb{Z}_{N}$
with eigenvalues $\lambda_{k}=4\sin^{2}(\pi k/N)$ and combinatorial
trace identities; the 1D building block for the $d$-dimensional
heat-kernel-trace factorisation used in the lattice-resonance audit.

\bibitem{LinWan2024}
Y.~Lin and S.~Wan,
\emph{Connection heat kernel on connection lattices and connection
discrete torus},
arXiv:2404.00668 (2024).
Establishes the Kronecker-sum theorem: the connection heat kernel
on an arbitrary high-dimensional lattice with product connection
equals the Kronecker sum of one-dimensional connection lattices'
heat kernels. Mathematically validates the factorisation
$M(N)=(S_{1d}(N)/N)^{d}$ used in
\pyown{src/verify_D_Omega_2adic_resonance.py}{verify_D_Omega_2adic_resonance} and
\pyown{src/verify_D_Omega_discrete_laplacian_spectrum.py}{verify_D_Omega_discrete_laplacian_spectrum}.

\bibitem{ChintaJK2010}
G.~Chinta, J.~Jorgenson and A.~Karlsson,
\emph{Zeta functions, heat kernels and spectral asymptotics on
degenerating families of discrete tori},
Nagoya Math.\ J.\ \textbf{198}, 121--172 (2010),
arXiv:0806.2014.
Spectral asymptotics of the combinatorial Laplacian on
degenerating discrete tori as the lattice sizes $N_{i}\to\infty$;
the zeta-regularised continuum-torus determinant appears as the
constant term in the asymptotic expansion of
$\log\det\Delta_{\rm latt}$, with arithmetic-dependent subleading
contributions analogous to the
$\sin^{2}\theta_{\rm chir}(N)\,v_{2}(N)$ residual.
